document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("input", function (e) {
        if (!e.target.matches(".cantidad-visible")) return;
...
PRELOAD (EDITAR)
============================ */
        const preloaded = window.preloadedProductos || [];

        if (preloaded.length > 0) {
            // limpia placeholder si ya hay cosas
            selectedTbody.innerHTML = "";
        }

        preloaded.forEach(item => {
            const rawId = item.id;
            const inventario = String(item.inv); // puede ser "combo"
            const cantidad = Number(item.cantidad ?? 1);
            const nombre = item.nombre ?? "";
            const referencia = item.codigo_qr ?? "";

            const key = `${rawId}__inv${inventario}`;

            // evitar duplicados
            if (selectedTbody.querySelector(`tr[data-key="${key}"]`)) return;

            const tr = document.createElement("tr");
            tr.dataset.key = key;
            tr.dataset.id = rawId;
            tr.dataset.inv = inventario;
            tr.dataset.is_insumo = item.is_insumo;
            const disabled = item.is_insumo !== true;

            // En preload NO dependemos de is_insumo (porque no tenemos btn.dataset)
            // Si quieres respetar el disabled por insumo, toca mandar is_insumo en el preload.
            tr.innerHTML = `
                <td>
                    <div class="fw-semibold">${escapeHtml(nombre)}</div>
                    <div><small class="text-muted">${escapeHtml(referencia)}</small></div>
    ...
    /* ============================
        PRECARGAR TABLA SUPERIOR (REMISIÓN) EN EDIT
    ============================ */
    if (window.detallesRemision && window.detallesRemision.length > 0) {
        cargarProductosDeRemision(window.detallesRemision);
        cargarProductosDisponiblesDesdeRemision(window.detallesRemision);
        rebuildSearchIndex();
    }
